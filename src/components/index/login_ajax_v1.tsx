...
interface EmployerInfo {
  RW_LAYOUT_TYPE?: number;
}

interface LoginAjaxV1Props {
  language?: 'vi' | 'en';
  LANGUAGE?: string;
  HOME?: string;
  TN?: string;
...
export default function LoginAjaxV1({
  language = 'vi',
  LANGUAGE = 'vi',
  HOME = '/hoasen',
  TN = '/hoasen',
  ...
  useEffect(() => {
    ...
    const initLoginForm = () => {
      if (typeof window !== 'undefined' && (window as any).jQuery) {
        ...
        // Determine CSS file path based on layout type
        const cssFile = arrEmployer.RW_LAYOUT_TYPE === 4 ? '../css/core.css' : 'css/core.css';
        
        // Initialize Fancybox for showDialog-login elements
        $(".showDialog-login").fancybox({
          onComplete: function() {
    ...
    initLoginForm();
  }, [HOME, LANGUAGE, TN, LOGIN_FAIL, arrEmployer.RW_LAYOUT_TYPE]);
  ...
  return (
    <>
      {/* Custom CSS for login image */}
      {linkImageLogin && (
        <style jsx>{`
          :root {
            --LinkImageLogin: url(${linkImageLogin});
          }
        `}</style>
      )}
              ...
              <div className="form-login">
                <form name="frmLogin" id="frmLogin" method="post" className="fontCore">
                  ...
                  <div className="form-group form-action">
                    <button className="ui_btnCb" type="submit" id="btnLogin">
                  ...
                  <>
                    ...
                    <div className="form-group">
                      {arrFunction.SHOW_JOIN_TS === 0 && arrFunction.SHOW_JOIN_COLLEGE === 1 ? (
                        ...
                        <a href={getLink('join')} className="btn-st2 btn-joints showDialog-login">
                          {t('_Join Our Talent Network_', language)}
                        </a>
                      )}
                    </div>

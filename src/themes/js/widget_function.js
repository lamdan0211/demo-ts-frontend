$(document).ready(function(){
	$('.showDialog_adv').click(function() {					
		$('#frm_calc_dynamic_ifrm_talentnetwork', parent.document).width(width_).height(height_);		
		$('#frm_calc_dynamic_talentnetwork', parent.document).css('left',0).css('top',0).css('position','absolute');		
		$.fancybox({ 
			href: "#AdvancedCalculator",	
			'onStart': function(){$('.BoxHolder').css('display', 'none')},
			'onClosed': function(){$('#frm_calc_dynamic_talentnetwork', parent.document).removeAttr('style');$('.BoxHolder').css('display', 'block');$('#frm_calc_dynamic_ifrm_talentnetwork', parent.document).width(_width).height(_height);}
		});	
		return true;
	});						
	...
	$('#social, #health, #unemployed, #trade_union').formatCurrency({ negativeFormat: '-%s%n', roundToDecimalPlace: 1 });	
...
function refeshValue()
{
	$.ajax({
		type: "POST",
		url: TN+"/widget/load-default-usd"
	}).done(function( msg ) {
		$('#default_usd').val(msg);
...
function calTax(total)
{
	...
	deep = 7;
	for(i=0; i<5; i++)
		arrRange[i] = parseInt(arrRange[i]);
	while(total > 0 && deep-- > 0) {
		tmp = 0;
		if(total <= arrRange[0]) {
			tmp = parseFloat((total) * 5 / 100);
			total = 0;
		} else if(total <= arrRange[1] && total > arrRange[0]) {
			tmp = parseFloat((total - arrRange[0]) * 10 / 100);
			total = arrRange[0];
		} else if(total <= arrRange[2] && total > arrRange[1]) {
			tmp = parseFloat((total - arrRange[1]) * 15 / 100);
			total = arrRange[1];
		} else if(total <= arrRange[3] && total > arrRange[2]) {
			tmp = parseFloat((total - arrRange[2]) * 20 / 100);
			total = arrRange[2];
		} else if(total <= arrRange[4] && total > arrRange[3]) {
			tmp = parseFloat((total - arrRange[3]) * 25 / 100);
			total = arrRange[3];
		} else if(total <= arrRange[5] && total > arrRange[4]) {
			tmp = parseFloat((total - arrRange[4]) * 30 / 100);
			total = arrRange[4];
		} else {
			tmp = parseFloat((total-arrRange[5]) * 35 / 100);
			total = arrRange[5];
		}
		ret = ret + tmp;
	}

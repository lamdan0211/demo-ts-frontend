(function($){
 $.fn.multicolselect = function(options) {
   ...
   var options = $.extend(defaults, options);
	...
	if ($.browser.msie){
		obj.before("<div><input type='text' name='user_id_map_cb' id='mltsel' value='"+options.defaultValue+"' ><input id='mltselbtn' type='image' src='"+options.buttonImage+"' style='position:absolute;margin-top:1px;clear:both;'/></div>");
		obj.css("background","#fff");
		obj.css("position","absolute");
		obj.css("z-index","2000");	
	}else{
		obj.before("<div><input type='text' name='user_id_map_cb' class='mltsel' value='"+options.defaultValue+"' style='width: 240px'><input id='mltselbtn' type='image' src='"+options.buttonImage+"' style='position:absolute;margin-left:-1px'/></div>");
		obj.css("background","#fff");
		obj.css("position","absolute");
		obj.css("z-index","2000");	
	}
